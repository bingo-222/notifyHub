...
经过考虑，初步的架构如下：
```
业务系统 → Gateway(API) → 持久化和消息队列 → Worker → 外部供应商 API
                ↓                        ↓
            管理后台                  监控告警
```
Gateway主要用于接受业务系统用户的请求，并进行持久化和把任务放入消息队列

Worker主要用于从消息队列中取任务，并根据消息来调用不同的供应商的API

在业务量加大时，Gateway和Worker的实例可以独立的横向扩展，不影响对方。另外在实现上Gateway和Worker要尽量和具体的消息队列解耦。

### AI使用说明
在刚开始做本项目的架构设计时，
- 作者已经有了将用于接受请求的gateway和后面用于向供应商发送通知的worker分开实现的想法，除了将需求和AI讨论之外，将这个想法也和AI进行了讨论，也得到了AI的认可，不仅是功能分离，并且分开实现确实有利于对gateway和worker的单独横向扩展
